...
def create_map(df_esb, df_scraper, df_matches):
    # Hitung center map dari semua data
    all_lats = np.concatenate([
        df_esb['latitude'].dropna(),
        df_scraper['latitude'].dropna(),
        df_matches['latitude_esb'].dropna()
    ])
    all_lons = np.concatenate([
        df_esb['longitude'].dropna(),
        df_scraper['longitude'].dropna(),
        df_matches['longitude_esb'].dropna()
    ])
    
    center_lat = np.mean(all_lats) if len(all_lats) > 0 else -6.2
    center_lon = np.mean(all_lons) if len(all_lons) > 0 else 106.8
...
def main():
    ...
    with tab2:
        ...
        with col1:
            st.metric("Exact Brand Matches", 
                     f"{filtered_matches['exact_brand_match'].sum():,}")
        
        with col2:
            st.metric("Average Distance", 
                     f"{filtered_matches['distance_m'].mean():.1f}m")
        
        with col3:
            st.metric("Average Confidence", 
                     f"{filtered_matches['match_confidence'].mean():.3f}")
    ...
    with tab3:
        ...
        st.dataframe(
            filtered_matches[[
                'brandName_esb', 'brandName_pulau', 'match_confidence', 
                'distance_m', 'address_common_words', 'match_level'
